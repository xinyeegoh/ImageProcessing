...
% Preliminaries. 
if nargin == 1  %check how many input arguments when calling this function
   dir = 'same';  
   conn = 8; 
elseif nargin == 2  
   dir = 'same'; 
...
% Begin processing. 
x0 = b(1, 1); %coordinate x
y0 = b(1, 2); %coordinate y
c.x0y0 = [x0, y0]; 
...
a = circshift(b, [-1, 0]);  %[row col] = [shift updown, shift leftright] = [-: up, +:down , -: left, +:right] 
...
% terminates. 
%why? because boundary should be connected : (9,1), (9,2), (9,3),....
% if the difference between 2 successive coordinates >1, it means not
% connected. the difference should always only be 1 or -1
...
fcc = C(z); %to generate the chain code
 
% Check if direction of code sequence needs to be reversed. 
%opposite direction, NOT reverse code
...
if conn == 4 
   ...
   if isempty(val) %no diagonal boundary 
      fcc = fcc./2; 
   else 
      warning('The specified 4-connected code cannot be satisfied.') %exists diagonal boundary
   end 
end 
...
% for the min starting point
c.mm = minmag(fcc); % See below for function minmag. 
 
% Obtain the first difference of fcc
%code difference for the rotation thingy
c.diffmm = codediff(c.mm, conn); 
 
%-------------------------------------------------------------------% 
function cr = coderev(fcc) %the direction cw ccw thingy
...
ind1 = find(0 <= cr & cr <= 3); %direction 0 replace previous direction 4
ind2 = find(4 <= cr & cr <= 7); 
cr(ind1) = cr(ind1) + 4; 
cr(ind2) = cr(ind2) - 4; 
 
%-------------------------------------------------------------------% 
function z = minmag(c) %the starting point thingy
%MINMAG Finds the integer of minimum magnitude in a chain code. 3405->0534
...
[M, N] = size(A); 
J = (1:M)'; %M = number of candidates
for k = 2:N 
   D(1:M, 1) = Inf; %create a tempo D vertical vector
   D(J, 1) = A(J, k); %copy the xth col of A to D
   amin = min(A(J, k)); %get the min point of the col
   J = find(D(:, 1) == amin); %see how many points having the min too
   if length(J)==1 
      z = A(J, :); 
      return 
   end 
...
%-------------------------------------------------------------------% 
function d = codediff(fcc, conn) %the rotation thingy
...
delta = sr - fcc; 
d = delta; 
I = find(delta < 0); %negative value means we need to go through the circle
%5 to 7 = 7-5 = 2, we just move forward
%7 to 5 = 5-7 = -2, we make a round
    % 7 to 5 means we need to go ccw from 7,0,1,2,3,4,5
    % we pass 6 codes, difference = 6
    % using the formula below : -2 + 8 = 6
